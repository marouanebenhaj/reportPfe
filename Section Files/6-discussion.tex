...
\noindent \newline
A second pattern concerns the direction of the FiLM effect on MSS within V2. Although FiLM improves parameter-space MAE in the five-fold results, the V2 no-pitch variants attain a marginally lower MSS than their FiLM counterparts on the shared set (ConvNeXt no-pitch $1.383$ vs FiLM $1.405$; AST no-pitch $1.273$ vs FiLM $1.292$), and this reversal disappears in V3, where the FiLM models attain the lower MSS as well. A plausible explanation is that under single-pitch training, where each preset is seen at only one spectral position, the FiLM pathway can exploit absolute spectral position as a shortcut rather than learning true pitch normalisation; on the shared evaluation set, which is also single-pitch per preset, the conditioned features then carry pitch-specific biases that inflate the audio-space reconstruction error even as parameter-space error falls. V3's multi-pitch training removes this shortcut by presenting the same parameter vector at several pitches, so the FiLM pathway is forced toward genuinely pitch-invariant representations and improves both metrics together.
